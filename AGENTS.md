...
## Fork task completion requirements

- `vp check` and `vp run typecheck` must pass before considering tasks completed.
  - If changing native mobile code, `vp run lint:mobile` must also pass.
- Use `vp test` for the built-in Vite+ test command and `vp run test` when you specifically need the `test` package script.
- If you make a code change to the repository, document it in [FORK.md](./FORK.md).
- Keep local verification focused on the files and packages changed. Backend changes must include focused tests for the changed behavior; run targeted formatting, lint, and type checks when available.
...
- Reserve the required repo-wide `vp check` and `vp run typecheck` for the final QA pass unless the user explicitly requests them earlier.
- After frontend feature development or any user-visible frontend behavior change, the primary agent must run one integrated verification pass for each affected client surface after integrating the work. This fork requirement overrides the browser-consent guidance below:
...
## Syncing this fork with the upstream T3 Code repository

Since this is a fork of the T3 Code repository, it is important to keep it in sync with the upstream repository.
But this process requires explicit user consent.
...
- Reserve the required repo-wide `vp check` and `vp run typecheck` for the final QA pass. Do not run other repo-wide test or typecheck commands unless asked.
