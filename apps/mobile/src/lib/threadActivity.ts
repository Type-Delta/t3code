...
export function buildThreadFeed(
  ...
  const loadedMessages = (options?.loadedMessages ?? thread.messages).filter(
    (message) => message.subagentId === undefined,
  );
  const messages = options?.localMessages
    ? [
        ...loadedMessages,
        ...options.localMessages.filter((message) => message.subagentId === undefined),
      ]
    : loadedMessages;
