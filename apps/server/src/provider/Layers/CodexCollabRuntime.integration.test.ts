...
function buildScript() {
  ...
  const extras = [
    ...
    },
    {
      ...
      params: { threadId: CHILD_A, threadName: "child-only name" },
    },
    {
    ...
    },
    {
      ...
      params: {
        threadId: CHILD_A,
        turnId: `${CHILD_A}-turn-1`,
        itemId: "child-plan-item",
        delta: "child-only delta",
      },
    },
    {
      ...
      params: {
        threadId: CHILD_A,
        turnId: `${CHILD_A}-turn-1`,
        diff: "diff --git a/child.ts b/child.ts\n+child-only\n",
      },
    },
    ...
    Effect.gen(function* () {
      ...
      for (const childOwnedMethod of [
        ...
        assert.notInclude(
          methods,
          childOwnedMethod,
          `${childOwnedMethod} must not mutate parent state or timeline`,
        );
      }
