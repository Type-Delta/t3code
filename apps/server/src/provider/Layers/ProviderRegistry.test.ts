...
it.layer(Layer.mergeAll(NodeServices.layer, ServerSettingsModule.layerTest(), TestHttpClientLive))(
  ...
  (it) => {
    ...
    describe("ProviderRegistryLive", () => {
      ...
      it("replaces a cached model list with a reduced authoritative inventory", () => {
        const previousProvider = {
          instanceId: ProviderInstanceId.make("codex"),
          driver: ProviderDriverKind.make("codex"),
          status: "ready",
      ...
      it("replaces cached models with an empty authoritative inventory", () => {
        const previousProvider = {
          instanceId: ProviderInstanceId.make("claudeAgent"),
          driver: ProviderDriverKind.make("claudeAgent"),
          status: "ready",
      ...
      it.effect("persists the merged snapshot when a live update has empty models", () =>
        Effect.gen(function* () {
          ...
          yield* Effect.gen(function* () {
            ...
            const registryUpdate = yield* registry.streamChanges.pipe(
              Stream.runHead,
              Effect.forkChild,
            );
            yield* PubSub.publish(changes, refreshedProvider);
            yield* Fiber.join(registryUpdate);
    ...
    describe("checkClaudeProviderStatus", () => {
      ...
      it.effect("runs Claude status probes with the configured Claude configuration", () => {
