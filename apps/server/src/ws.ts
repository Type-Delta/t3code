...
const makeWsRpcLayer = (
  ...
  WsRpcGroup.toLayer(
    Effect.gen(function* () {
      ...
      const threadCommandDispatcher = yield* ThreadCommandDispatcher.ThreadCommandDispatcher;
      ...
      const dispatchOptions = hasClientOrigin ? { origin: clientOrigin } : undefined;
      const dispatchFromClient: OrchestrationEngine.OrchestrationEngineShape["dispatch"] = (
        command,
      ) => orchestrationEngine.dispatch(command, dispatchOptions);
        ...
        [ORCHESTRATION_WS_METHODS.dispatchCommand]: (command) =>
          observeRpcEffect(
            ...
            Effect.gen(function* () {
              ...
              // after either lands. The decider rejects settling an active
              // session, so settle only stops an idle one.
              ...
              if (parkingCommand) {
                ...
                if (shouldStopSessionAfterCommand) {
                  yield* Effect.gen(function* () {
                    ...
                    yield* threadCommandDispatcher.dispatch(stopCommand, dispatchOptions);
                  }).pipe(
                    Effect.catchCause((cause) =>
                ...
                // terminal panes close with it. Settled threads stay reachable.
