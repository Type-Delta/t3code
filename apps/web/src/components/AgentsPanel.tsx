...
const AgentTranscriptContext = createContext<{
  readonly ids: ReadonlySet<string>;
  readonly open: (agentId: string) => void;
} | null>(null);
const EMPTY_TRANSCRIPT_AGENT_IDS: ReadonlySet<string> = new Set();
...
function AgentRow({ agent }: { agent: RuntimeSubagent }) {
  const transcript = useContext(AgentTranscriptContext);
  const canOpenTranscript = transcript?.ids.has(agent.id) === true;
  ...
  return canOpenTranscript ? (
    <button
      type="button"
      onClick={() => transcript.open(agent.id)}
...
export function AgentsPanel({
  ...
  transcriptAgentIds = EMPTY_TRANSCRIPT_AGENT_IDS,
}: {
  model: AgentPanelModel;
  environmentId?: EnvironmentId | null;
  threadId?: ThreadId | null;
  onOpenTranscript?: ((agentId: string) => void) | null;
  transcriptAgentIds?: ReadonlySet<string>;
}) {
  const transcriptContext = useMemo(
    () => (onOpenTranscript === null ? null : { ids: transcriptAgentIds, open: onOpenTranscript }),
    [onOpenTranscript, transcriptAgentIds],
  );
  if (!model.hasAgents) {
    return (
  ...
  return (
    <AgentTranscriptContext.Provider value={transcriptContext}>
