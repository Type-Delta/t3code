...
import { useActiveComposerHandle } from "../composerHandleContext";
import {
...
interface ChatViewPaneMode {
  ...
  /** Workspace-owned portal destination for the active pane's right panel. */
...
interface ChatViewSharedProps {
  ...
  /** Set only when ChatView is rendered inside SplitThreadWorkspace. */
  paneMode?: ChatViewPaneMode;
}
...
function ChatViewContent(props: ChatViewProps) {
  ...
  // In split mode every pane owns a local composer handle; only the focused
  // pane forwards it to the command palette's app-wide handle.
  const composerRef = useActiveComposerHandle({
    scoped: isPaneMode,
    active: isActivePane,
  });
  ...
  // The shell carries server PR updates while thread detail is still loading.
  ...
  const canMaximizeRightPanel = rightPanelOpen && !shouldUseRightPanelSheet && !isPaneMode;
  ...
  }, [closePullRequestDialog, isActivePane]);
  ...
  const mainThreadActivities = useMemo(
    () =>
      threadActivities.filter(
        (activity) =>
          ...
          !subagentTranscriptIds.has(activity.subagentId) ||
    ...
    [subagentTranscriptIds, threadActivities],
  );
  ...
  const timelineMessages = useMemo(() => {
    const messages = displayServerMessages.filter(
      (message) =>
        message.subagentId === undefined || !subagentTranscriptIds.has(message.subagentId),
    );
      ...
      for (let index = runIds.length - 1; index >= 0; index -= 1) {
        ...
        useRightPanelStore
          ...
          .openSubagent(
            activeThreadRef,
            runId,
            findSubagentInPanelModel(agentPanelModel, runId)?.title ??
              subagentRunsById.get(runId)?.title ??
              "Subagent",
          );
      }
    },
    [activeThreadRef, agentPanelModel, subagentRunsById],
  );
  const openSubagentTranscript = useCallback(
    (agentId: string) => openSubagentRuns([agentId]),
    [openSubagentRuns],
  );
  ...
  // Paste-to-focus: a resting composer may be blurred while the timeline has
  // focus, so route plain text pastes back into the active pane's composer.
  useEffect(() => {
  ...
  }, [activeThreadId, composerRef, isActivePane]);
  ...
  const onCompactContext = async () => {
    ...
    const compactThreadId = activeThread.id;
    const messageId = newMessageId();
    const createdAt = new Date().toISOString();
    sendInFlightRef.current = true;
    beginLocalDispatch();
    setThreadError(compactThreadId, null);
  ...
  // A split pane can disappear while reconciliation is removing it. Its
  // workspace already owns the inset, so never render the standalone empty
  // state (which creates another SidebarInset) from a pane.
  if (!activeThread) {
    return isPaneMode ? null : <NoActiveThreadState />;
  }
  ...
  const activeSubagentRun =
    renderedRightPanelSurface?.kind === "subagent"
      ? resolveSubagentPanelRun({
          runId: renderedRightPanelSurface.runId,
          surfaceTitle: renderedRightPanelSurface.title,
          legacyRun: subagentRunsById.get(renderedRightPanelSurface.runId),
          nativeAgent: findSubagentInPanelModel(agentPanelModel, renderedRightPanelSurface.runId),
  ...
  const rightPanelContent = activeThreadRef ? (
    renderedRightPanelSurface?.kind === "preview" ? (
    ...
    ) : renderedRightPanelSurface?.kind === "subagent" && activeSubagentRun ? (
      <SubagentPanel
      ...
      <AgentsPanel
        ...
        onOpenTranscript={openSubagentTranscript}
        transcriptAgentIds={subagentTranscriptIds}
      />
  ...
  const headerSlot = paneMode?.headerSlot ?? null;
  const rightPanelSlot = paneMode?.rightPanelSlot ?? null;
  const chatHeader = (
    <>
      ...
      <WorkspacePageHeader
        ...
        className={cn("relative bg-background", isPaneMode && "w-full")}
      >
  ...
  const renderedChatHeader =
    isPaneMode && isActivePane && headerSlot
      ? createPortal(chatHeader, headerSlot)
      : isPaneMode
        ? null
        : chatHeader;
    ...
    <>
      {rightPanelPresent && !shouldUseRightPanelSheet && activeThreadRef ? (
        <RightPanelTabs
          ...
          open={rightPanelOpen}
          maximized={rightPanelMaximized}
          sourceThread={{ key: activeThreadKey ?? routeThreadKey, title: activeThread.title }}
  ...
  return (
    <div
      ...
      {...(isPaneMode
        ? {
            onPointerDownCapture: () => paneMode?.onActivate(),
            onFocusCapture: () => paneMode?.onActivate(),
          }
        : {})}
    >
      <div
      ...
      >
        ...
        <div className="flex min-h-0 min-w-0 flex-1">
          ...
          >
            ...
            {isActivePane && pullRequestDialogState ? (
