  ...
  for (let index = 0; index < input.timelineEntries.length; index += 1) {
    ...
    if (timelineEntry.kind === "work") {
      ...
      if (visibleGroupedEntries.length > 0) {
        const containsSubagentEntry = visibleGroupedEntries.some(
          (entry) => (entry.subagentRunIds?.length ?? 0) > 0,
        );
        // Subagent rows carry actionable identity and metadata (and open the
        // read-only transcript panel), so keep them as visible work rows
        // instead of hiding them behind the generic "N tool calls" toggle.
        const activeInProgressToolEntries = visibleGroupedEntries.filter(workEntryIsInActiveRun);
        if (containsSubagentEntry) {
          nextRows.push({
            kind: "work",
            id: timelineEntry.id,
            createdAt: timelineEntry.createdAt,
