...
const PlainWorkEntryRow = memo(function PlainWorkEntryRow(props: {
  ...
  const basePreviewText = displayLabel ?? workEntryDisplayLabel(workEntry, workspaceRoot);
  const previewText = [
    basePreviewText,
    subagentMetadata(workEntry),
    workEntry.subagentPrompt?.trim() === basePreviewText ? null : workEntry.subagentPrompt?.trim(),
  ]
    .filter(Boolean)
    .join(" - ");
    ...
    >
      <div className="flex select-none items-center gap-1.5 transition-[opacity,translate] duration-200">
        ...
        <div className="flex min-w-0 flex-1 items-center gap-1.5">
          ...
          >
            {opensSubagent ? (
              <ChevronRightIcon className="size-3 shrink-0 text-icon-muted opacity-70" />
            ) : (
              <ChevronDownIcon
                className={cn(
          ...
          {showSuccessIndicator || showNeutralIndicator ? (
            <Tooltip>
              <TooltipTrigger render={<span className="flex size-4 items-center justify-center" />}>
                {showSuccessIndicator ? (
                  <CheckIcon className="block size-3 shrink-0" aria-hidden />
                ) : (
                  <MinusIcon className="block size-3 shrink-0 opacity-70" aria-hidden />
                )}
              </TooltipTrigger>
              <TooltipPopup>{showSuccessIndicator ? "Completed" : "Empty"}</TooltipPopup>
            </Tooltip>
          ) : null}
        </div>
      </div>
