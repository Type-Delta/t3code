...
export function deriveProviderModelsForDisplay(input: {
  ...
  const customModelSet = new Set(input.customModels.map((entry) => entry.slug));
  const serverModels =
    input.liveModels
      ?.filter(
        (model) =>
      ...
      .map((model) => ({
        ...model,
        name: input.modelOverrides?.[model.slug]?.displayName ?? model.name,
      })) ?? [];
  const customModels = input.customModels.map((entry) => ({
    slug: entry.slug,
    name:
      entry.name === entry.slug
        ? (input.modelOverrides?.[entry.slug]?.displayName ?? entry.name)
        : entry.name,
    isCustom: true,
    capabilities:
...
export function ProviderInstanceCard({
  ...
  const updateModelOverrides = (nextOverrides: Readonly<Record<string, ModelMetadataOverride>>) => {
    const nextConfig = nextConfigBlobWithValue(instance.config, "modelOverrides", nextOverrides);
    const { config: _omit, ...rest } = instance;
    onUpdate({ ...rest, config: nextConfig } as ProviderInstanceConfig);
  };
        ...
        >
          <div className="px-3 py-3 sm:px-4">
            <ProviderModelsSection
              ...
              onModelOverridesChange={updateModelOverrides}
