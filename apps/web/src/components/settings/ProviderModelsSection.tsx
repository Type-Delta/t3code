...
interface ProviderModelsSectionProps {
  ...
  readonly onModelOverridesChange: (next: Readonly<Record<string, ModelMetadataOverride>>) => void;
...
export function ProviderModelsSection({
  ...
  const [metadataSlug, setMetadataSlug] = useState<string | null>(null);
  ...
  const handleRemove = (slug: string) => {
    ...
    if (modelOverrides[slug] !== undefined) {
      const nextOverrides = { ...modelOverrides };
      delete nextOverrides[slug];
      onModelOverridesChange(nextOverrides);
    }
        ...
        <TooltipPopup side="top" className="w-80 max-w-[calc(100vw-1rem)] text-left">
          <div className="grid grid-cols-[auto_minmax(0,1fr)] gap-x-3 gap-y-1 text-[11px] leading-snug">
            {deriveProviderModelDetails({
              model,
              override: modelOverrides[model.slug],
              capabilityLabels: describeModelCapabilities(model),
            }).map((detail) => (
        ...
        <TooltipTrigger
          render={
            <Button
              size="icon-micro"
              variant="ghost-muted"
              aria-label={`Edit ${model.name} metadata`}
              onClick={() => setMetadataSlug(model.slug)}
            />
          }
        >
          <PencilIcon className="size-3" />
        </TooltipTrigger>
        <TooltipPopup side="top">Edit limits and metadata</TooltipPopup>
      </Tooltip>
      {customModels.some((entry) => entry.slug === model.slug) ? (
        <>
          <Tooltip>
            <TooltipTrigger
              render={
    ...
    <div className="lg:flex lg:h-full lg:min-h-0 lg:flex-col">
      ...
      {metadataSlug ? (
        <CustomModelMetadataDialog
          open
          slug={metadataSlug}
          initialValue={modelOverrides[metadataSlug]}
          mode="edit"
          onOpenChange={(open) => {
            if (!open) setMetadataSlug(null);
          }}
          onSave={(value) =>
            onModelOverridesChange({
              ...modelOverrides,
