...
export type RightPanelSurface =
  ...
  | { id: `subagent:${string}`; kind: "subagent"; runId: string; title: string }
  | {
      /**
...
// v12 retains fork subagent transcript surfaces alongside pull-request surfaces.
...
interface RightPanelStoreState {
  ...
  open: (
    ref: ScopedThreadRef,
    kind: Exclude<RightPanelKind, "file" | "terminal" | "subagent" | "pull-request">,
  ) => void;
  ...
  openSubagent: (ref: ScopedThreadRef, runId: string, title: string) => void;
  splitTerminal: (
    ref: ScopedThreadRef,
    surfaceId: string,
    terminalId: string,
...
const singletonSurface = (
  kind: Exclude<RightPanelKind, "file" | "preview" | "terminal" | "subagent" | "pull-request">,
): RightPanelSurface => {
  switch (kind) {
    case "diff":
      return { id: "diff", kind };
...
const subagentSurface = (runId: string, title: string): RightPanelSurface => ({
  id: `subagent:${runId}`,
  kind: "subagent",
  runId,
  title,
...
export const useRightPanelStore = create<RightPanelStoreState>()(
  persist(
    (set) => ({
      ...
      openSubagent: (ref, runId, title) =>
        set((state) => ({
          byThreadKey: updateThread(state.byThreadKey, scopedThreadKey(ref), (current) =>
            upsertSurface(current, subagentSurface(runId, title)),
          ),
        })),
