...
const navigationCompleted = (sequence: number): OrchestrationThreadStreamItem => ({
  kind: "event",
  event: {
    eventId: EventId.make(`event-navigation-${sequence}`),
    sequence,
    occurredAt: "2026-04-01T01:45:00.000Z",
    commandId: null,
    causationEventId: null,
    correlationId: null,
    metadata: {},
...
describe("thread pagination state", () => {
  ...
  it.effect("checkpoint navigation refreshes the current window outside older-page work", () =>
    Effect.gen(function* () {
      const refreshed: OrchestrationThreadDetailSnapshot = {
        snapshotSequence: 11,
        thread: { ...BASE_THREAD, title: "After undo" },
        page: { beforeCursor: "cursor-2", hasMore: true, snapshotSequence: 11 },
      };
      const harness = yield* makeHarness({
        cached: WINDOWED_SNAPSHOT,
        initialResponse: Option.some(refreshed),
      });
      yield* harness.awaitState((value) => Option.isSome(value.page));
      ...
      yield* Queue.offer(harness.inputs, navigationCompleted(11));
      const state = yield* harness.awaitState(
        (value) =>
          Option.isSome(value.data) &&
          value.data.value.title === "After undo" &&
          Option.isSome(value.page) &&
          value.page.value.beforeCursor === "cursor-2",
      );
      ...
      // The page started before navigation belongs to the previous history
      // epoch and cannot be merged after the authoritative replacement.
      yield* harness.resolveNextPage(Option.some(OLDER_PAGE));
  ...
  it.effect("checkpoint navigation refreshes the accumulated loaded window", () =>
    Effect.gen(function* () {
      const refreshed: OrchestrationThreadDetailSnapshot = {
        snapshotSequence: 11,
        thread: { ...BASE_THREAD, title: "After expanded undo" },
        page: { beforeCursor: null, hasMore: false, snapshotSequence: 11 },
      };
      const harness = yield* makeHarness({
        cached: WINDOWED_SNAPSHOT,
        initialResponse: Option.some(refreshed),
      ...
      yield* harness.awaitState(
        (value) => Option.isSome(value.data) && value.data.value.title === "After expanded undo",
      );
  ...
  it.effect("rehydrates the persisted window width for checkpoint refresh", () =>
    Effect.gen(function* () {
      const cached: OrchestrationThreadDetailSnapshot = {
        ...WINDOWED_SNAPSHOT,
        page: { ...WINDOWED_SNAPSHOT.page!, turnLimit: 70 },
      };
      const refreshed: OrchestrationThreadDetailSnapshot = {
        snapshotSequence: 11,
        thread: { ...BASE_THREAD, title: "After cached undo" },
        page: {
          beforeCursor: "cursor-after-undo",
          hasMore: true,
          snapshotSequence: 11,
          turnLimit: 70,
        },
      ...
      yield* Queue.offer(harness.inputs, navigationCompleted(11));
      yield* harness.awaitState(
        (value) => Option.isSome(value.data) && value.data.value.title === "After cached undo",
      );

      expect(yield* Ref.get(harness.loaderWindows)).toEqual([{ turnLimit: 70 }]);
    }),
  );
