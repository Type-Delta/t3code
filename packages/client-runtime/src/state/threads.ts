...
function snapshotUserTurnLimit(snapshot: OrchestrationThreadDetailSnapshot): number | null {
  return snapshot.page === undefined
    ? null
    : (snapshot.page.turnLimit ?? INITIAL_THREAD_USER_TURN_LIMIT);
}
...
export const makeEnvironmentThreadState = Effect.fn("EnvironmentThreadState.make")(function* (
  ...
  const remember = Effect.gen(function* () {
    ...
    const turnLimit = yield* Ref.get(loadedUserTurnLimit);
    committed = {
      state: current,
      sequence,
      turnLimit,
  ...
  // The page cursor describes where the loaded window starts, not how wide it
  // is. Keep the requested width separately so authoritative checkpoint
  // refreshes can replace the same window instead of downloading full history.
  const loadedUserTurnLimit = yield* Ref.make<number | null>(
    Option.match(cached, {
      onNone: () => null,
      onSome: snapshotUserTurnLimit,
    }),
  ...
  const offerThreadPersistence = Effect.fn("EnvironmentThreadState.offerThreadPersistence")(
    function* (thread: OrchestrationThread, snapshotSequence: number) {
      const [currentPage, currentTurnLimit] = yield* Effect.all([
        SubscriptionRef.get(state).pipe(Effect.map((value) => value.page)),
        Ref.get(loadedUserTurnLimit),
      ]);
          ...
          onSome: (value) =>
            ({
              page: {
                ...
                snapshotSequence,
                ...(currentTurnLimit === null ? {} : { turnLimit: currentTurnLimit }),
              },
            }) as const,
        }),
      });
  ...
  const refetchThreadDetail = Effect.fn("EnvironmentThreadState.refetchThreadDetail")(function* (
    ...
    const windowLimit = yield* Ref.get(loadedUserTurnLimit);
    const supportsPagination = yield* Ref.get(paginationSupported);
    const epochAtStart = yield* Ref.get(historyEpoch);
    const refreshed = yield* snapshotLoader.load(
      prepared.value,
      threadId,
      supportsPagination && windowLimit !== null ? { turnLimit: windowLimit } : undefined,
      yield* Ref.get(reasoningMessagesSupported),
    );

    // HTTP stays outside applyLock. Only the staleness check and replacement
    // are serialized with live events and older-page merges.
    yield* applyLock.withPermits(1)(
      Effect.gen(function* () {
        if (Option.isNone(refreshed)) return;
        const currentSequence = yield* SubscriptionRef.get(lastSequence);
        if (
          (yield* Ref.get(historyEpoch)) !== epochAtStart ||
          refreshed.value.snapshotSequence < minimumSequence ||
          refreshed.value.snapshotSequence < currentSequence
        ) {
          return;
        }
        yield* Ref.update(historyEpoch, (epoch) => epoch + 1);
        ...
        yield* Ref.set(
          loadedUserTurnLimit,
          refreshed.value.page === undefined ? null : windowLimit,
        );
        yield* SubscriptionRef.set(lastSequence, refreshed.value.snapshotSequence);
        yield* setThread(refreshed.value.thread, pageStateFromSnapshot(refreshed.value.page));
      }),
    );
  });
  ...
  const applyItemLocked = Effect.fn("EnvironmentThreadState.applyItemLocked")(function* (
    ...
    if (item.kind === "snapshot") {
      ...
      yield* Ref.update(historyEpoch, (epoch) => epoch + 1);
      yield* Ref.set(loadedUserTurnLimit, snapshotUserTurnLimit(item.snapshot));
      // The parked page belongs to the replaced history window.
      yield* Ref.set(pendingOlderPage, null);
    ...
    if (
      ...
      // Invalidate older-page work immediately. The authoritative refresh
      // runs outside this lock in applyItem.
      yield* Ref.update(historyEpoch, (epoch) => epoch + 1);
      yield* Ref.set(pendingOlderPage, null);
    }
  ...
  const applyItem = Effect.fn("EnvironmentThreadState.applyItem")(function* (
    ...
    if (
      ...
      // server. Refetch the current window rather than synthesizing that
  ...
  const applyItems = Effect.fn("EnvironmentThreadState.applyItems")(function* (
    ...
    yield* applyLock.withPermits(1)(
      Effect.gen(function* () {
        ...
        // A settled state can be followed by the next turn in the same batch.
  ...
  const mergeOlderPage = Effect.fn("EnvironmentThreadState.mergeOlderPage")(function* (
    ...
    const accumulatedTurnLimit =
      snapshot.page === undefined
        ? yield* Ref.get(loadedUserTurnLimit)
        : yield* Ref.modify(loadedUserTurnLimit, (current) => {
            const next =
              current === null
                ? INITIAL_THREAD_USER_TURN_LIMIT
                : current + OLDER_THREAD_PAGE_USER_TURN_LIMIT;
        ...
        ...(snapshot.page === undefined
          ...
          : {
              page: {
                ...snapshot.page,
                snapshotSequence,
                turnLimit: accumulatedTurnLimit ?? INITIAL_THREAD_USER_TURN_LIMIT,
              },
            }),
      });
    }
        ...
        if (!supportsPagination) {
          yield* applyLock.withPermits(1)(
            Effect.gen(function* () {
              ...
              yield* Ref.update(historyEpoch, (epoch) => epoch + 1);
              yield* Ref.set(loadedUserTurnLimit, null);
              yield* SubscriptionRef.update(state, (value) => ({
                ...value,
                data: Option.none(),
  ...
  yield* Effect.addFinalizer(() =>
    Effect.suspend(() => {
      const { state: current, sequence: snapshotSequence, turnLimit } = committed;
