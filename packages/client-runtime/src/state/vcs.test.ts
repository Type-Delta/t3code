...
import {
  ...
  makeVcsWorktreesChanges,
  vcsWorktreesRevisionAtom,
} from "./vcs.ts";
import {
...
describe("cached VCS refs", () => {
  ...
  it.effect("invalidates refs and worktree queries when commands settle", () =>
    Effect.scoped(
      Effect.gen(function* () {
        ...
        const client = {
          ...
          [WS_METHODS.vcsRemoveWorktree]: () => Effect.void,
        } as unknown as WsRpcProtocolClient;
        const supervisor = EnvironmentSupervisor.EnvironmentSupervisor.of({
          target: TARGET,
          state: yield* SubscriptionRef.make(CONNECTED_CONNECTION_STATE),
        ...
        yield* Effect.promise(() =>
          atoms.createWorktree.run(registry, {
            environmentId: TARGET.environmentId,
            input: { cwd: "/repo", refName: "main", newRefName: "feature", path: "/worktree" },
          }),
        );
        expect(yield* Ref.get(clears)).toBe(3);
        expect(registry.get(vcsWorktreesRevisionAtom(TARGET.environmentId))).toBe(1);

        yield* Effect.promise(() =>
          atoms.removeWorktree.run(registry, {
            environmentId: TARGET.environmentId,
            input: { cwd: "/repo", path: "/worktree" },
          }),
        );
        expect(yield* Ref.get(clears)).toBe(4);
        expect(registry.get(vcsWorktreesRevisionAtom(TARGET.environmentId))).toBe(2);
      }),
    ),
  );
...
describe("VCS worktrees", () => {
  it.effect("refreshes once per connection generation without periodic polling", () =>
    Effect.scoped(
      Effect.gen(function* () {
        const requests = yield* Ref.make<ReadonlyArray<VcsListWorktreesInput>>([]);
        const connectionState = yield* SubscriptionRef.make(CONNECTED_CONNECTION_STATE);
        const client = {
          [WS_METHODS.vcsListWorktrees]: (input: VcsListWorktreesInput) =>
            Ref.update(requests, (current) => [...current, input]).pipe(
              Effect.as({ worktrees: [], isRepo: true }),
            ),
        } as unknown as WsRpcProtocolClient;
        const supervisor = EnvironmentSupervisor.EnvironmentSupervisor.of({
          target: TARGET,
        ...
        const stream = Stream.unwrap(
          makeVcsWorktreesChanges({ cwd: "/repo", refresh: true }).pipe(
            Effect.provideService(EnvironmentSupervisor.EnvironmentSupervisor, supervisor),
          ),
        ).pipe(Stream.runDrain);
        const fiber = yield* Effect.forkChild(stream);
        ...
        for (
          let attempt = 0;
          attempt < 100 && (yield* Ref.get(requests)).length < 1;
          attempt += 1
        ) {
          yield* Effect.yieldNow;
        }
        expect(yield* Ref.get(requests)).toEqual([{ cwd: "/repo", refresh: true }]);
        ...
        for (
          let attempt = 0;
          attempt < 100 && (yield* Ref.get(requests)).length < 2;
          attempt += 1
        ) {
          yield* Effect.yieldNow;
        }
        expect(yield* Ref.get(requests)).toEqual([
          { cwd: "/repo", refresh: true },
          { cwd: "/repo", refresh: true },
        ]);
        yield* Fiber.interrupt(fiber);
      }).pipe(Effect.provide(TestClock.layer())),
